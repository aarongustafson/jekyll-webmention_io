(function(window, JekyllWebmentionIO){
  ...
  function identifyWebmentionCollections()
  {
    ...
    while ( w-- )
    {
      $webmention_collection = $webmentions_groups[w];
  ...
  function doleOutWebmentions( webmentions )
  {
    ...
    while( i--)
    {
      addWebmentionsToCollection( incoming[queue_keys[i]], webmention_receivers[queue_keys[i]] );
    }
  ...
  {
    console.log( 'incoming!', mentions, $webmention_collection );
    ...
    // append
    $list.innerHTML += template.parse(mentions);
  }
  ...
  function removeDuplicates( webmentions )
  {
    ...
    while ( i-- )
    {
      if ( existing_webmentions.indexOf( webmentions[i].id ) < 0 )
      {
