...
(function(window, document){
  ...
  // Gathers embedded templates.
  //
  // Templates are emitted by the gem as
  //   <script type="text/x-template" id="webmention-{type}">...HTML...</script>
  // rather than <template> elements so their content stays opaque to
  // microformats2 parsers (the spec says <template> is inert, but some parsers
  // walk it anyway and surface the unrendered Liquid placeholders as garbage
  // microformats). We parse the script's textContent into a detached DOM tree
  // so the rest of the pipeline can keep treating templates as live elements.
  function collectTemplates()
  {
    var $templates = document.querySelectorAll( 'script[type="text/x-template"][id^=webmention-]' ),
        t = $templates.length,
        $template, $container, $ol;
    ...
    while ( t-- )
    {
      ...
      if ( $ol )
      {
        templates[$template.id] = $ol;
      }
      else
      {
        templates[$template.id] = $container;
      }
    }
    $template = null;
  }
