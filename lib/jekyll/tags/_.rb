...
module Jekyll
  ...
  class WebmentionTag < Liquid::Tag
    ...
    def set_template(template)
      supported_templates = WebmentionIO.types + %w[count webmentions]

      WebmentionIO.log 'error', "#{template} is not supported" unless supported_templates.include? template

      if WebmentionIO.config.key?('templates') && WebmentionIO.config['templates'].key?(template)
